...
# python3 /home/limdoyeon/capstone_4_1/src/four_wheel_robot/four_wheel_robot/kinematics_node.py
...
class KinematicsNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('dxl_center', 2048)        # 조향 중앙 틱값 (4:1 기어비 반영, 위치확장모드)
        self.declare_parameter('steer_min_deg', -45.0)     # 최소 조향각
        self.declare_parameter('steer_max_deg', 45.0)      # 최대 조향각
        self.declare_parameter('max_steer_ticks', 512.0)    # 최대 조향각(steer_max_deg)에 도달하기 위한 틱 오프셋 (4:1 기어비 반영)

        self.declare_parameter('max_wheel_rpm', 10.0)  
        ...
        self.DXL_MIN    = float(self.get_parameter('steer_min_deg').value)
        self.DXL_MAX    = float(self.get_parameter('steer_max_deg').value)
        self.MAX_STEER_TICKS = float(self.get_parameter('max_steer_ticks').value)
        self.TICKS_PER_DEG = self.MAX_STEER_TICKS / abs(self.DXL_MAX) if self.DXL_MAX != 0 else 0.0

        self.SPEED_MAX = float(self.get_parameter('max_wheel_rpm').value)
        ...
        self.mode_subscription = self.create_subscription(Int32, 'control_mode', self.mode_callback, 10)
        self.current_mode = 1
...
# =======================================================================================================================================
    ...
    def mode_callback(self, msg):
        self.current_mode = max(1, min(3, int(msg.data)))

    def publish_command_arrays(self, wheel_speeds, steering_angles):
        angle_msg = Float32MultiArray()
        angle_msg.data = [float(x) for x in steering_angles]
        self.steering_angle_pub.publish(angle_msg)

        speed_msg = Float32MultiArray()
        speed_msg.data = [float(x) for x in wheel_speeds]
    ...
    def vel_callback(self, msg):
        ...
        if self.current_mode == 2:
            lateral_speed = self.map_value(lateral, -1.0, 1.0, self.SPEED_MIN, self.SPEED_MAX)
            wheel_speeds = [lateral_speed, lateral_speed, -lateral_speed, -lateral_speed]
            steering_angles = [1024,3072,3072,1024]
            self.publish_command_arrays(wheel_speeds, steering_angles)
            return

        if self.current_mode == 3:
            spin_speed = self.map_value(abs(w), 0.0, 1.0, 0.0, self.SPEED_MAX)
            if w >= 0:
                wheel_speeds = [float(spin_speed), float(spin_speed), float(spin_speed), float(spin_speed)]
            else:
                wheel_speeds = [float(-spin_speed), float(-spin_speed), float(-spin_speed), float(-spin_speed)]
            steering_angles = [2510, 1586, 1586, 2510]
            self.publish_command_arrays(wheel_speeds, steering_angles)
            return
...
# =======================================================================================================================================
        ...
        if w == 0:
            ...
            wheel_speeds = [speed, -speed, -speed, speed]                                              # 4개 바퀴의 구동 속도를 모두 위에서 계산한 동일한 speed 값으로 설정합니다.
...
# =======================================================================================================================================
            ...
            '''
            w(각속도)가 아주 작은 값(예: 0.00001)일 때 map_value 결과가

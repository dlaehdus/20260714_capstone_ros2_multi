...
class MotorDriverNode(Node):
    def __init__(self):
        ...
        self.driver_configs = [
            ...
            {'port': '/dev/serial/by-id/usb-WCH.CN_USB_Quad_Serial_BCD9B6ABCD-if00', 'id': 1},
            
            # 후륜 구동 모터 드라이버가 추가된다면 아래와 같이 추가 (if02 등)
            {'port': '/dev/serial/by-id/usb-WCH.CN_USB_Quad_Serial_BCD9B6ABCD-if02', 'id': 1},
        ]
    ...
    def check_no_command(self):
        ...
        if diff > 333.0 and not self.motor_stopped:
